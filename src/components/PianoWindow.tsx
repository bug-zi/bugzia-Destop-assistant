...
function resolveTones(key: string, mode: PlayMode): { midi: number; label: string }[] | null {
  ...
  if (!note) return null;
  if (mode === "free") return [{ midi: note.midi, label: note.note }];
  const chordRoot = note.midi >= 72 ? note.midi - 12 : note.midi;
  return [
...
export default function PianoWindow() {
  ...
  const legend = mode === "free" ? "自然音自由演奏" : "大三和弦伴奏";
  ...
  const refreshActiveKeys = useCallback(() => {
    setActiveKeys(new Set([...activeSoundsRef.current.values()].map((sound) => sound.visualKey)));
  }, []);
  ...
  const startSound = useCallback(
    (
      ...
      if (timbreRef.current === 1) {
        ...
        void Promise.all(
          tones.map((tone) => createGrandPianoVoice(context, tone.midi, velocity, amplitude)),
        )
          .then((voices) => {
      ...
      releaseVoices = tones.map((tone) =>
        createVoice(context, midiToFrequency(tone.midi), preset, amplitude),
      );
    },
  ...
  }, [releaseAll, releaseSound, releaseSustainedSounds, startSound]);
    ...
    <main className="piano-window">
      ...
      <section className="piano-stage">
        <div className="piano-note-summary">
          <span>{mode === "free" ? "C3–B5，自然音" : "C3–B5，每键一个大三和弦"}</span>
          <strong>
        ...
        <div className="piano-keyboard" aria-label={legend}>
          {keyRows.map((row, rowIndex) => (
            <div className="piano-key-row" key={KEY_ROWS[rowIndex].octave}>
              {row.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className="piano-key-note">
                      {mode === "free" ? item.note : `${chordName} 大`}
                    </span>
